#!/usr/bin/env bash
# Run grid comparing LR vs SCALE for cnn and mlp
#
# Usage example:
#   STEPS=50 BATCH=512 JIT=1 bash run_grid_lr_scale.sh
#
...
#   - Per-model CSVs written as results_lr_vs_scale_<label>.csv
...
for m in "${MODELS[@]}"; do
  ...
  for lr in "${LRS[@]}"; do
    for pat in "${SCALE[@]}"; do
      ...
      if [ "$DRY_RUN" = "1" ]; then
        echo "[DRY_RUN] would run: model=$label LR=$lr SCALE=$pat -> $cmd"
      else
        echo "[RUN ${label}_${run_idx}] model=$label LR=$lr SCALE=$pat -> running..."
        $cmd
        if [ -f run_last.log ]; then
          # include the parameter name in the filename so downstream parsing can find it (e.g. _scale5)
