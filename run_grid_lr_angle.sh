#!/usr/bin/env bash
# Run grid comparing LR vs ANGLE for cnn and mlp
#
# Usage example:
...
#   - Per-model CSVs written as results_lr_vs_angle_<label>.csv
...
for m in "${MODELS[@]}"; do
  ...
  for lr in "${LRS[@]}"; do
    for ang in "${ANGLES[@]}"; do
      ...
      if [ "$DRY_RUN" = "1" ]; then
        echo "[DRY_RUN] would run: model=$label LR=$lr ANGLE=$ang -> $cmd"
      else
        echo "[RUN ${label}_${run_idx}] model=$label LR=$lr ANGLE=$ang -> running..."
        $cmd
        if [ -f run_last.log ]; then
          mv run_last.log "$LOGDIR/run_${label}_${run_idx}_lr${lr_f}_ang${ang_f}_s${STEPS}.log"
        fi
        if [ -f scripts/append_from_log.py ]; then
          lastlog="$LOGDIR/run_${label}_${run_idx}_lr${lr_f}_ang${ang_f}_s${STEPS}.log"
